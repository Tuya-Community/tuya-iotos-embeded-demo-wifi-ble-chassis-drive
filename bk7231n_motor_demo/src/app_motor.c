 ...
 * @LastEditTime: 2021-03-25 17:17:36
 */
...
OPERATE_RET app_motor_init(IN APP_MODE mode)
{
    ...
    if(APP_NORMAL == mode) {
        ...
        tuya_hal_thread_create(NULL, "sensor_data_get_thread", 512*4, TRD_PRIO_4, sensor_data_get_thread, NULL);
...
VOID app_report_all_dp_status(VOID)
{
    ...
    dp_arr[0].dpid = DPID_MOTOR_STATE;
    dp_arr[0].type = PROP_ENUM;
    dp_arr[0].time_stamp = 0;
    dp_arr[0].value.dp_value = app_ctrl_data.motor_state;

    dp_arr[1].dpid = DPID_MOTOR_SPEED;
    dp_arr[1].type = PROP_VALUE;
    dp_arr[1].time_stamp = 0;
    dp_arr[1].value.dp_value = app_ctrl_data.motor_speed;
...
VOID deal_dp_proc(IN CONST TY_OBJ_DP_S *root)
{
    ...
    case DPID_MOTOR_STATE:
        app_ctrl_data.motor_state = root->value.dp_enum;
        break;
    case DPID_MOTOR_SPEED:
        app_ctrl_data.motor_speed = root->value.dp_value;
        break;
    default:
        break;
    }
